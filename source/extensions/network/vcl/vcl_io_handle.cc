...
#include "source/extensions/network/vcl/vcl_event.h"
  ...
  while (n_bytes < rv) {
    ...
    auto fragment = new Envoy::Buffer::BufferFragmentImpl(
        ...
        [&, sh, len](const void*, size_t, const Envoy::Buffer::BufferFragmentImpl* this_fragment) {
          // fprintf (stdout, "sh %x freed %u\n", sh, len);
          vppcom_session_free_segments(sh, len);
          delete this_fragment;
        });

    buffer.addBufferFragment(*fragment);
    // fprintf (stdout, "sh %x added %u\n", sh, len);
    n_bytes += len;
    ds_index += 1;
  }

  // fprintf(stderr, "bytes:%u\n%s\n", rv, buffer.toString().c_str());
  return vclCallResultToIoCallResult(rv);
}

// Api::IoCallUint64Result VclIoHandle::read(Buffer::Instance& buffer, uint64_t max_length) {
//   if (max_length == 0) {
...
Api::SysCallIntResult VclIoHandle::ioctl(unsigned long, void*,
                                         unsigned long, void*,
                                         unsigned long,
                                         unsigned long* ) {
  return {0, 0};
