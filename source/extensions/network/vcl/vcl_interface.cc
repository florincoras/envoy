#include "source/extensions/network/vcl/vcl_interface.h"

#include "envoy/extensions/network/socket_interface/v3/vcl_socket_interface.pb.h"

#include "source/common/network/address_impl.h"

#include "source/extensions/network/vcl/vcl_io_handle.h"
...
static void onMqSocketEvents(uint32_t flags) {
  ...
  while (max_events > 0) {
    ...
    for (int i = 0; i < n_events; i++) {
      ...
      //   fprintf(stderr, "wrk_index %u worker listener sh %x ", wrk_index, vcl_handle->sh());
      if (vcl_handle->isWrkListener()) {
        vcl_handle = vcl_handle->getParentListener();
        // fprintf(stderr, "parent handle %x\n", vcl_handle->sh());
      }
...
void vcl_maybe_activate_event() {
  // if (vppcom_worker_n_unhandled_evts()) {
  //   VCL_LOG("had unhandled\n");
  //   mq_events_[vppcom_worker_index()]->activate(Event::FileReadyType::Read);
  // }
}
...
Envoy::Network::IoHandlePtr VclSocketInterface::socket(Envoy::Network::Socket::Type socket_type,
  ...
  // return std::make_unique<VclIoHandle>(static_cast<uint32_t>(sh), vppcom_mq_epoll_fd());
